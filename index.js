...
function addFlavor(flav, inf){
    for (let i = 0; i < inf.length; i++){
        return inf.unshift(flav);
    }
...
function removeLastFlavor(lizt){
...
function getFlavorByIndex(lizt, int){
    let resul = [];
    for (let i = 0; i < lizt.length; i++){
        resul.push(lizt[int]);
        return resul;
...
function removeFlavorByName(lizt, strin){
        lizt.splice(lizt.indexOf(strin), 1);
        return lizt;
    
//    for (let i = 0; i < lizt.length; i++){ TEST
//        lizt.splice(30, 1);                TEST
//        return lizt;                       TEST
//    }
...
function copy(newAr, oldAr){
        newAr = oldAr.slice(0, 40); 
        return newAr;
...
function filterByWord(arra, sting){
        let newArra = [];
    for (let i = 0; i < arra.length; i++){
        if (arra[i].includes(sting)){
            newArra.push(arra[i]);
            console.log( newArra);
        }       
    }
